@echo off
setlocal
REM Repair-WindowsSystemFiles.cmd  -  launcher for Repair-WindowsSystemFiles.ps1  (CHANGES WINDOWS, asks for admin)
REM Shows the report and saves a copy under C:\Temp\Toolkit. Set TOOLKIT_NOPAUSE=1 to skip the final pause.
set "PS1=%~dp0Repair-WindowsSystemFiles.ps1"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%PS1%" -Display -LogPath "%OUTDIR%"
set "RC=%ERRORLEVEL%"
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Start-Process powershell.exe -Verb RunAs -ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-NoExit','-File',('\"{0}\"' -f $env:PS1),'-Display','-LogPath',('\"{0}\"' -f $env:OUTDIR.TrimEnd('\')))"
if errorlevel 1 (
