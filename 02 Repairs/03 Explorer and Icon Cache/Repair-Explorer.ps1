<#
.SYNOPSIS
    Restart Explorer and optionally remove the selected user's icon/thumbnail cache.
.EXAMPLE
...
param([switch]$Display,[string]$LogPath='C:\Temp\Toolkit',[string]$TargetUser,[switch]$RestartOnly)
$ErrorActionPreference='Stop'
# ---------------------------------------------------------------- toolkit helpers (verbatim, do not edit)
$script:ToolName  = 'Repair-Explorer'
...
$cacheDir=Assert-LocalRepairPath (Join-Path $local 'Microsoft\Windows\Explorer') $local
$caches=@()
if(-not $RestartOnly){
    $icon=Join-Path $local 'IconCache.db'
    if(Test-Path -LiteralPath $icon -PathType Leaf){$caches+=Get-Item -LiteralPath (Assert-LocalRepairPath $icon $local) -Force}
    if(Test-Path -LiteralPath $cacheDir -PathType Container){
        $caches+=@(Get-ChildItem -LiteralPath $cacheDir -File -Force -ErrorAction Stop | Where-Object {$_.Name -like 'iconcache_*.db' -or $_.Name -like 'thumbcache_*.db'})
    }
    foreach($cache in $caches){$null=Assert-LocalRepairPath $cache.FullName $local}
}
$processes=@(Get-TargetProcesses 'explorer.exe' $user)
$mb=Round1 (($caches | Measure-Object Length -Sum).Sum/1MB)
$preview=[bool]$WhatIfPreference
$script:RepairLog=New-RepairLog $LogPath (Join-Path $local 'Temp') 'ShellRepair' $preview
$plan="Close Explorer in session $($user.SessionId), remove $($caches.Count) cache files ($mb MB) from $local, and restart Explorer."
Write-RepairLog $plan
foreach($f in $caches){Write-RepairLog ('Planned file: '+$f.FullName)}
$o=[ordered]@{ComputerName=$env:COMPUTERNAME;CollectedAt=[datetime]::Now;WhatIf=$preview;Performed=$false;TargetUser=$user.Name;TargetSessionId=[int]$user.SessionId;RunningAs=$user.RunningAs;Elevated=(Test-IsAdmin);ExplorerProcessesStopped=0;CacheFilesFound=$caches.Count;CacheFilesMB=$mb;CacheFilesDeleted=0;CacheFilesLocked='';ExplorerRelaunched=$false;RelaunchMethod='Not attempted';ExplorerRunningAfter=[bool]$processes.Count;LogPath=$script:RepairLog;NextStep='Preview or declined; Explorer and caches unchanged';Warnings=''}
try{$go=$PSCmdlet.ShouldProcess(("$($user.Name), session $($user.SessionId), $($user.ProfilePath)"),$plan)}catch{throw 'Refusing: this host cannot show the confirmation prompt. Run interactively or pass -WhatIf.'}
$locked=New-Object System.Collections.Generic.List[string]
if($go){
    $o.Performed=$true
    try {
        foreach($p in @(Get-TargetProcesses 'explorer.exe' $user)){
            Stop-Process -Id $p.ProcessId -Force -Confirm:$false -ErrorAction Stop;$o.ExplorerProcessesStopped++
        }
        Start-Sleep -Seconds 2
        foreach($f in $caches){
            try{
                $safe=Assert-LocalRepairPath $f.FullName $local
                if(Test-Path -LiteralPath $safe -PathType Leaf){Remove-Item -LiteralPath $safe -Force -Confirm:$false -ErrorAction Stop;$o.CacheFilesDeleted++;Write-RepairLog ('Deleted: '+$safe)}
            }catch{
                if($_.CategoryInfo.Category -eq [Management.Automation.ErrorCategory]::ObjectNotFound -and -not (Test-Path -LiteralPath $f.FullName -PathType Leaf -ErrorAction SilentlyContinue)){
                    Write-RepairLog ('Already absent after Explorer restart: '+$f.FullName)
                }else{$locked.Add($f.Name+': '+$_.Exception.Message)}
            }
        }
    }catch{Add-Warning ('Shell repair: '+$_.Exception.Message)}
    finally {
        try{
            if(@(Get-TargetProcesses 'explorer.exe' $user).Count){$o.RelaunchMethod='Windows restarted Explorer';$o.ExplorerRelaunched=$true}
            else{$o.RelaunchMethod=Start-TargetApplication (Join-Path $env:windir 'explorer.exe') 'explorer.exe' $user;$o.ExplorerRelaunched=$true}
        }catch{Add-Warning ('Explorer restart: '+$_.Exception.Message);$o.RelaunchMethod='Manual steps given'}
        $o.ExplorerRunningAfter=[bool]@(Invoke-Section 'Explorer check' {Get-TargetProcesses 'explorer.exe' $user} -Default @()).Count
    }
    $o.NextStep='Check the taskbar and icons; caches rebuild as you use Explorer'
    if(-not $o.ExplorerRunningAfter){$o.NextStep='In the affected session: Ctrl+Shift+Esc, Run new task, enter explorer.exe'}
    elseif($locked.Count){$o.NextStep='Some caches were locked. Check the log; sign out and back in if the problem remains'}
}
$o.CacheFilesLocked=$locked -join '; ';$o.Warnings=$script:Warnings -join '; ';$result=[pscustomobject]$o
Write-RepairLog ($result | ConvertTo-Json -Depth 4)
